...
export async function detectColorFromImage(base64ImageData: string): Promise<ColorResult> {
  if (!process.env.API_KEY) {
    throw new Error("La API KEY de Google Gemini no está configurada. Añádela como variable de entorno en Vercel.");
  }
  ...
  try {
    ...
    if (!jsonString) {
      throw new Error("La IA devolvió una respuesta vacía. Intenta enfocar mejor el objeto.");
    }

    const result: ColorResult = JSON.parse(jsonString);

    if (!/^#[0-9A-F]{6}$/i.test(result.hexCode)) {
      throw new Error(`Formato de código hexadecimal inválido: ${result.hexCode}`);
    }
    ...
    throw new Error("La IA no pudo identificar el color. Inténtalo de nuevo.");
  }
}
